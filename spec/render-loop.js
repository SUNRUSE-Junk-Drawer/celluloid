describe("renderLoop", () => {
  ...
  describe("with render loop checks", () => {
    let requestAnimationFrame, cancelAnimationFrame, shouldStartRenderLoop
    beforeEach(() => {
      ...
      shouldStartRenderLoop = jasmine.createSpy("shouldStartRenderLoop")
      index.__set__("shouldStartRenderLoop", shouldStartRenderLoop)
    })

    describe("when a render loop should not be started", () => {
      beforeEach(() => shouldStartRenderLoop.and.returnValue(false))
      ...
      describe("when a render loop is not running", () => {
        describe("startRenderLoop", () => {
          ...
          it("does not request an animation frame", () => expect(requestAnimationFrame).not.toHaveBeenCalled())
          it("does not cancel an animation frame", () => expect(cancelAnimationFrame).not.toHaveBeenCalled())
          it("does not modify the stored animation frame", () => expect(index.__get__("renderLoopAnimationFrame")).toBeNull())
    ...
    describe("when a render loop should be started", () => {
      beforeEach(() => shouldStartRenderLoop.and.returnValue(true))
      describe("when a render loop is running", () => {
        beforeEach(() => {
          index.__set__("renderLoopAnimationFrame", "Test Existing Animation Frame")
          index.__set__("renderLoopLastTimestamp", 8973.21)
